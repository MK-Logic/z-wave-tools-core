...
namespace ZWave.CommandClasses
{
    public partial class COMMAND_CLASS_USER_CREDENTIAL_V3
    {
        ...
        public partial class CREDENTIAL_CAPABILITIES_REPORT
        {
            ...
            public class TVG9NUMBEROFCREDENTIALSLOTSPERUSER
            {
                public const byte numberOfCredentialSlotsPerUserBytesCount = 2;
                public byte[] numberOfCredentialSlotsPerUser = new byte[numberOfCredentialSlotsPerUserBytesCount];
            }
            public List<TVG9NUMBEROFCREDENTIALSLOTSPERUSER> vg9NumberOfCredentialSlotsPerUser = new List<TVG9NUMBEROFCREDENTIALSLOTSPERUSER>();
            public static implicit operator CREDENTIAL_CAPABILITIES_REPORT(byte[] data)
            {
                ...
                if (data != null)
                {
                    ...
                    ret.vg9NumberOfCredentialSlotsPerUser = new List<TVG9NUMBEROFCREDENTIALSLOTSPERUSER>();
                    for (int j = 0; j < ret.numberOfSupportedCredentialTypes; j++)
                    {
                        TVG9NUMBEROFCREDENTIALSLOTSPERUSER tmp = new TVG9NUMBEROFCREDENTIALSLOTSPERUSER();
                        tmp.numberOfCredentialSlotsPerUser = (data.Length - index) >= TVG9NUMBEROFCREDENTIALSLOTSPERUSER.numberOfCredentialSlotsPerUserBytesCount ? new byte[TVG9NUMBEROFCREDENTIALSLOTSPERUSER.numberOfCredentialSlotsPerUserBytesCount] : new byte[data.Length - index];
                        if (data.Length > index) tmp.numberOfCredentialSlotsPerUser[0] = data[index++];
                        if (data.Length > index) tmp.numberOfCredentialSlotsPerUser[1] = data[index++];
                        ret.vg9NumberOfCredentialSlotsPerUser.Add(tmp);
                    }
                }
                return ret;
            }
            public static implicit operator byte[](CREDENTIAL_CAPABILITIES_REPORT command)
            {
                ...
                if (command.vg9NumberOfCredentialSlotsPerUser != null)
                {
                    foreach (var item in command.vg9NumberOfCredentialSlotsPerUser)
                    {
                        if (item.numberOfCredentialSlotsPerUser != null)
                        {
                            foreach (var tmp in item.numberOfCredentialSlotsPerUser)
                            {
                                ret.Add(tmp);
                            }
                        }
        ...
        public partial class KEY_LOCKER_CAPABILITIES_GET
        {
            public const byte ID = 0x1D;
            public static implicit operator KEY_LOCKER_CAPABILITIES_GET(byte[] data)
            {
                KEY_LOCKER_CAPABILITIES_GET ret = new KEY_LOCKER_CAPABILITIES_GET();
                return ret;
            }
        ...
        public partial class KEY_LOCKER_CAPABILITIES_REPORT
        {
            public const byte ID = 0x1E;
            public ByteValue numberOfSupportedEntryTypes = 0;
            public class TVG1SUPPORTEDENTRYTYPE
            {
                public ByteValue supportedEntryType = 0;
            }
            public List<TVG1SUPPORTEDENTRYTYPE> vg1SupportedEntryType = new List<TVG1SUPPORTEDENTRYTYPE>();
            public class TVG2NUMBEROFENTRYSLOTSPERENTRYTYPE
            {
                public const byte numberOfEntrySlotsBytesCount = 2;
                public byte[] numberOfEntrySlots = new byte[numberOfEntrySlotsBytesCount];
            }
            public List<TVG2NUMBEROFENTRYSLOTSPERENTRYTYPE> vg2NumberOfEntrySlotsPerEntryType = new List<TVG2NUMBEROFENTRYSLOTSPERENTRYTYPE>();
            public class TVG3MINLENGTHOFENTRYDATAPERENTRYTYPE
            {
                public const byte minLengthOfEntryDataBytesCount = 2;
                public byte[] minLengthOfEntryData = new byte[minLengthOfEntryDataBytesCount];
            }
            public List<TVG3MINLENGTHOFENTRYDATAPERENTRYTYPE> vg3MinLengthOfEntryDataPerEntryType = new List<TVG3MINLENGTHOFENTRYDATAPERENTRYTYPE>();
            public class TVG4MAXLENGTHOFENTRYDATAPERENTRYTYPE
            {
                public const byte maxLengthOfEntryDataBytesCount = 2;
                public byte[] maxLengthOfEntryData = new byte[maxLengthOfEntryDataBytesCount];
            }
            public List<TVG4MAXLENGTHOFENTRYDATAPERENTRYTYPE> vg4MaxLengthOfEntryDataPerEntryType = new List<TVG4MAXLENGTHOFENTRYDATAPERENTRYTYPE>();
            public static implicit operator KEY_LOCKER_CAPABILITIES_REPORT(byte[] data)
            {
                KEY_LOCKER_CAPABILITIES_REPORT ret = new KEY_LOCKER_CAPABILITIES_REPORT();
                if (data != null)
                {
                    int index = 2;
                    ret.numberOfSupportedEntryTypes = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
                    ret.vg1SupportedEntryType = new List<TVG1SUPPORTEDENTRYTYPE>();
                    for (int j = 0; j < ret.numberOfSupportedEntryTypes; j++)
                    {
                        TVG1SUPPORTEDENTRYTYPE tmp = new TVG1SUPPORTEDENTRYTYPE();
                        tmp.supportedEntryType = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
                        ret.vg1SupportedEntryType.Add(tmp);
                    }
                    ret.vg2NumberOfEntrySlotsPerEntryType = new List<TVG2NUMBEROFENTRYSLOTSPERENTRYTYPE>();
                    for (int j = 0; j < ret.numberOfSupportedEntryTypes; j++)
                    {
                        TVG2NUMBEROFENTRYSLOTSPERENTRYTYPE tmp = new TVG2NUMBEROFENTRYSLOTSPERENTRYTYPE();
                        tmp.numberOfEntrySlots = (data.Length - index) >= TVG2NUMBEROFENTRYSLOTSPERENTRYTYPE.numberOfEntrySlotsBytesCount ? new byte[TVG2NUMBEROFENTRYSLOTSPERENTRYTYPE.numberOfEntrySlotsBytesCount] : new byte[data.Length - index];
                        if (data.Length > index) tmp.numberOfEntrySlots[0] = data[index++];
                        if (data.Length > index) tmp.numberOfEntrySlots[1] = data[index++];
                        ret.vg2NumberOfEntrySlotsPerEntryType.Add(tmp);
                    }
                    ret.vg3MinLengthOfEntryDataPerEntryType = new List<TVG3MINLENGTHOFENTRYDATAPERENTRYTYPE>();
                    for (int j = 0; j < ret.numberOfSupportedEntryTypes; j++)
                    {
                        TVG3MINLENGTHOFENTRYDATAPERENTRYTYPE tmp = new TVG3MINLENGTHOFENTRYDATAPERENTRYTYPE();
                        tmp.minLengthOfEntryData = (data.Length - index) >= TVG3MINLENGTHOFENTRYDATAPERENTRYTYPE.minLengthOfEntryDataBytesCount ? new byte[TVG3MINLENGTHOFENTRYDATAPERENTRYTYPE.minLengthOfEntryDataBytesCount] : new byte[data.Length - index];
                        if (data.Length > index) tmp.minLengthOfEntryData[0] = data[index++];
                        if (data.Length > index) tmp.minLengthOfEntryData[1] = data[index++];
                        ret.vg3MinLengthOfEntryDataPerEntryType.Add(tmp);
                    }
                    ret.vg4MaxLengthOfEntryDataPerEntryType = new List<TVG4MAXLENGTHOFENTRYDATAPERENTRYTYPE>();
                    for (int j = 0; j < ret.numberOfSupportedEntryTypes; j++)
                    {
                        TVG4MAXLENGTHOFENTRYDATAPERENTRYTYPE tmp = new TVG4MAXLENGTHOFENTRYDATAPERENTRYTYPE();
                        tmp.maxLengthOfEntryData = (data.Length - index) >= TVG4MAXLENGTHOFENTRYDATAPERENTRYTYPE.maxLengthOfEntryDataBytesCount ? new byte[TVG4MAXLENGTHOFENTRYDATAPERENTRYTYPE.maxLengthOfEntryDataBytesCount] : new byte[data.Length - index];
                        if (data.Length > index) tmp.maxLengthOfEntryData[0] = data[index++];
                        if (data.Length > index) tmp.maxLengthOfEntryData[1] = data[index++];
                        ret.vg4MaxLengthOfEntryDataPerEntryType.Add(tmp);
                    }
                }
                return ret;
            }
            public static implicit operator byte[](KEY_LOCKER_CAPABILITIES_REPORT command)
            {
                ...
                {
                    foreach (var item in command.vg1SupportedEntryType)
                    {
                        if (item.supportedEntryType.HasValue) ret.Add(item.supportedEntryType);
                    }
                }
                if (command.vg2NumberOfEntrySlotsPerEntryType != null)
                {
                    foreach (var item in command.vg2NumberOfEntrySlotsPerEntryType)
                    {
                        if (item.numberOfEntrySlots != null)
                        {
                            foreach (var tmp in item.numberOfEntrySlots)
                            {
                                ret.Add(tmp);
                            }
                        }
                ...
                if (command.vg3MinLengthOfEntryDataPerEntryType != null)
                {
                    foreach (var item in command.vg3MinLengthOfEntryDataPerEntryType)
                    {
                        if (item.minLengthOfEntryData != null)
                        {
                            foreach (var tmp in item.minLengthOfEntryData)
                            {
                                ret.Add(tmp);
                            }
                        }
                ...
                if (command.vg4MaxLengthOfEntryDataPerEntryType != null)
                {
                    foreach (var item in command.vg4MaxLengthOfEntryDataPerEntryType)
                    {
                        if (item.maxLengthOfEntryData != null)
                        {
                            foreach (var tmp in item.maxLengthOfEntryData)
                            {
                                ret.Add(tmp);
                            }
                        }
        ...
        public partial class KEY_LOCKER_ENTRY_SET
        {
            ...
            public static implicit operator KEY_LOCKER_ENTRY_SET(byte[] data)
            {
                KEY_LOCKER_ENTRY_SET ret = new KEY_LOCKER_ENTRY_SET();
                if (data != null)
                {
                    int index = 2;
                    ret.entryType = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
        ...
        public partial class KEY_LOCKER_ENTRY_GET
        {
            public const byte ID = 0x20;
            public ByteValue entryType = 0;
            public const byte entrySlotBytesCount = 2;
            public byte[] entrySlot = new byte[entrySlotBytesCount];
            public static implicit operator KEY_LOCKER_ENTRY_GET(byte[] data)
            {
                KEY_LOCKER_ENTRY_GET ret = new KEY_LOCKER_ENTRY_GET();
                if (data != null)
                {
                    int index = 2;
                    ret.entryType = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
        ...
        public partial class KEY_LOCKER_ENTRY_REPORT
        {
