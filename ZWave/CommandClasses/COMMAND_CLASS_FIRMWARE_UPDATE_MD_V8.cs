...
{
    public partial class COMMAND_CLASS_FIRMWARE_UPDATE_MD_V8
    {
        ...
        public partial class FIRMWARE_MD_GET
        {
            ...
            public static implicit operator byte[](FIRMWARE_MD_GET command)
            {
                ...
                ret.Add(COMMAND_CLASS_FIRMWARE_UPDATE_MD_V8.ID);
                ret.Add(ID);
                return ret.ToArray();
            }
        }
        ...
        public partial class FIRMWARE_UPDATE_MD_PREPARE_GET
        {
        ...
        public partial class FIRMWARE_UPDATE_MD_PREPARE_REPORT
        {
            public const byte ID = 0x0B;
            public ByteValue status = 0;
            public const byte firmwareChecksumBytesCount = 2;
            public byte[] firmwareChecksum = new byte[firmwareChecksumBytesCount];
            public static implicit operator FIRMWARE_UPDATE_MD_PREPARE_REPORT(byte[] data)
            {
                ...
                if (data != null)
                {
                    int index = 2;
                    ret.status = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
                    ret.firmwareChecksum = (data.Length - index) >= firmwareChecksumBytesCount ? new byte[firmwareChecksumBytesCount] : new byte[data.Length - index];
                    if (data.Length > index) ret.firmwareChecksum[0] = data[index++];
                    if (data.Length > index) ret.firmwareChecksum[1] = data[index++];
                }
                return ret;
            }
            public static implicit operator byte[](FIRMWARE_UPDATE_MD_PREPARE_REPORT command)
            {
                ...
                ret.Add(ID);
                if (command.status.HasValue) ret.Add(command.status);
                if (command.firmwareChecksum != null)
                {
                    foreach (var tmp in command.firmwareChecksum)
                    {
                        ret.Add(tmp);
                    }
                }
