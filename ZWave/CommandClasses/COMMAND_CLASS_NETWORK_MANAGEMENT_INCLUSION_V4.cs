...
{
    public partial class COMMAND_CLASS_NETWORK_MANAGEMENT_INCLUSION_V4
    {
        ...
        public partial class NODE_ADD_STATUS
        {
            ...
            public static implicit operator NODE_ADD_STATUS(byte[] data)
            {
                ...
                if (data != null)
                {
                    ...
                    ret.reserved1 = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
                    ...
                    for (int i = 0; i < ret.properties3.dskLength; i++)
                    {
            ...
            public static implicit operator byte[](NODE_ADD_STATUS command)
            {
                ...
                if (command.reserved1.HasValue) ret.Add(command.reserved1);
        ...
        public partial class SMART_START_JOIN_STARTED_REPORT
        {
            ...
            public Tproperties1 properties1 = 0;
            public IList<byte> dsk = new List<byte>();
            public static implicit operator SMART_START_JOIN_STARTED_REPORT(byte[] data)
            {
                SMART_START_JOIN_STARTED_REPORT ret = new SMART_START_JOIN_STARTED_REPORT();
                if (data != null)
                {
                    int index = 2;
                    ret.seqNo = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
            ...
            public static implicit operator byte[](SMART_START_JOIN_STARTED_REPORT command)
            {
                List<byte> ret = new List<byte>();
                ret.Add(COMMAND_CLASS_NETWORK_MANAGEMENT_INCLUSION_V4.ID);
                ret.Add(ID);
        ...
        public partial class EXTENDED_NODE_ADD_STATUS
        {
            ...
            public ByteValue seqNo = 0;
            public ByteValue status = 0;
            public const byte assignedNodeidBytesCount = 2;
            public byte[] assignedNodeid = new byte[assignedNodeidBytesCount];
            public ByteValue nodeInfoLength = 0;
            public struct Tproperties1
            {
                private byte _value;
            ...
            public ByteValue grantedKeys = 0;
            public ByteValue kexFailType = 0;
            public static implicit operator EXTENDED_NODE_ADD_STATUS(byte[] data)
            {
                EXTENDED_NODE_ADD_STATUS ret = new EXTENDED_NODE_ADD_STATUS();
                if (data != null)
                {
                    int index = 2;
                    ret.seqNo = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
                    ret.status = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
                    ret.assignedNodeid = (data.Length - index) >= assignedNodeidBytesCount ? new byte[assignedNodeidBytesCount] : new byte[data.Length - index];
                    if (data.Length > index) ret.assignedNodeid[0] = data[index++];
                    if (data.Length > index) ret.assignedNodeid[1] = data[index++];
                    ret.nodeInfoLength = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
                    ret.properties1 = data.Length > index ? (Tproperties1)data[index++] : Tproperties1.Empty;
                    ret.properties2 = data.Length > index ? (Tproperties2)data[index++] : Tproperties2.Empty;
                    ret.basicDeviceClass = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
            ...
            public static implicit operator byte[](EXTENDED_NODE_ADD_STATUS command)
            {
                List<byte> ret = new List<byte>();
                ret.Add(COMMAND_CLASS_NETWORK_MANAGEMENT_INCLUSION_V4.ID);
                ret.Add(ID);
                if (command.seqNo.HasValue) ret.Add(command.seqNo);
                if (command.status.HasValue) ret.Add(command.status);
                if (command.assignedNodeid != null)
                {
                    foreach (var tmp in command.assignedNodeid)
                    {
                        ret.Add(tmp);
                    }
                }
