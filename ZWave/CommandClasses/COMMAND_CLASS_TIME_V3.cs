...
{
    public partial class COMMAND_CLASS_TIME_V3
    {
        ...
        public partial class DATE_GET
        {
            ...
            public static implicit operator byte[](DATE_GET command)
            {
                ...
                ret.Add(COMMAND_CLASS_TIME_V3.ID);
                ret.Add(ID);
                return ret.ToArray();
            }
        }
        ...
        public partial class DATE_SET
        {
            ...
            public ByteValue month = 0;
            public ByteValue day = 0;
            public static implicit operator DATE_SET(byte[] data)
            {
                DATE_SET ret = new DATE_SET();
                if (data != null)
                {
                    int index = 2;
                    ret.year = (data.Length - index) >= yearBytesCount ? new byte[yearBytesCount] : new byte[data.Length - index];
            ...
            public static implicit operator byte[](DATE_SET command)
            {
                List<byte> ret = new List<byte>();
                ret.Add(COMMAND_CLASS_TIME_V3.ID);
                ret.Add(ID);
        ...
        public partial class TIME_SET
        {
            ...
            public ByteValue second = 0;
            public static implicit operator TIME_SET(byte[] data)
            {
                TIME_SET ret = new TIME_SET();
                if (data != null)
                {
                    int index = 2;
                    ret.properties1 = data.Length > index ? (Tproperties1)data[index++] : Tproperties1.Empty;
                    ret.minute = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
                    ret.second = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
                }
                return ret;
            }
            public static implicit operator byte[](TIME_SET command)
            {
                List<byte> ret = new List<byte>();
                ret.Add(COMMAND_CLASS_TIME_V3.ID);
                ret.Add(ID);
                ...
                if (command.second.HasValue) ret.Add(command.second);
                return ret.ToArray();
            }
        }
    }
