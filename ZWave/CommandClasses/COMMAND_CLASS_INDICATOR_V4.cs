...
{
    public partial class COMMAND_CLASS_INDICATOR_V4
    {
        ...
        public partial class INDICATOR_SET
        {
            ...
            public static implicit operator byte[](INDICATOR_SET command)
            {
                ...
                ret.Add(COMMAND_CLASS_INDICATOR_V4.ID);
                ret.Add(ID);
        ...
        public partial class INDICATOR_DESCRIPTION_GET
        {
            public const byte ID = 0x06;
            public ByteValue indicatorId = 0;
            public static implicit operator INDICATOR_DESCRIPTION_GET(byte[] data)
            {
                INDICATOR_DESCRIPTION_GET ret = new INDICATOR_DESCRIPTION_GET();
                if (data != null)
                {
                    int index = 2;
                    ret.indicatorId = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
            ...
            public static implicit operator byte[](INDICATOR_DESCRIPTION_GET command)
            {
                List<byte> ret = new List<byte>();
                ret.Add(COMMAND_CLASS_INDICATOR_V4.ID);
                ret.Add(ID);
        ...
        public partial class INDICATOR_DESCRIPTION_REPORT
        {
            public const byte ID = 0x07;
            public ByteValue indicatorId = 0;
            public ByteValue descriptionLength = 0;
            public IList<byte> description = new List<byte>();
            public static implicit operator INDICATOR_DESCRIPTION_REPORT(byte[] data)
            {
                INDICATOR_DESCRIPTION_REPORT ret = new INDICATOR_DESCRIPTION_REPORT();
                if (data != null)
                {
                    int index = 2;
                    ret.indicatorId = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
                    ret.descriptionLength = data.Length > index ? (ByteValue)data[index++] : ByteValue.Empty;
                    ret.description = new List<byte>();
                    for (int i = 0; i < ret.descriptionLength; i++)
                    {
                        if (data.Length > index) ret.description.Add(data[index++]);
                    }
                }
                return ret;
            }
            public static implicit operator byte[](INDICATOR_DESCRIPTION_REPORT command)
            {
                List<byte> ret = new List<byte>();
                ret.Add(COMMAND_CLASS_INDICATOR_V4.ID);
                ret.Add(ID);
                if (command.indicatorId.HasValue) ret.Add(command.indicatorId);
                if (command.descriptionLength.HasValue) ret.Add(command.descriptionLength);
                if (command.description != null)
                {
                    foreach (var tmp in command.description)
                    {
                        ret.Add(tmp);
                    }
                }
